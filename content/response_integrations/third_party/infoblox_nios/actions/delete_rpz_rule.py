...
def main():
    ...
    siemplify.script_name = DELETE_RPZ_RULE_SCRIPT_NAME
    ...
    try:
        ...
        _ = api_manager.delete_rpz_rule(reference_id)
        output_message = f"Successfully deleted the RPZ Rule with reference ID 1{reference_id}1."
        ...
        status = EXECUTION_STATE_FAILED
        output_message = COMMON_ACTION_ERROR_MESSAGE.format(DELETE_RPZ_RULE_SCRIPT_NAME, str(e))
        result_value = RESULT_VALUE_FALSE
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
