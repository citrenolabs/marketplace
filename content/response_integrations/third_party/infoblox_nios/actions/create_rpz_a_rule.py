...
from ..core.constants import (
    ...
    CREATE_RPZ_A_RULE_ACTION_IDENTIFIER_DOMAIN,
    CREATE_RPZ_A_RULE_ACTION_IDENTIFIER_IP,
    CREATE_RPZ_A_RULE_SCRIPT_NAME,
    RESULT_VALUE_FALSE,
    RESULT_VALUE_TRUE,
)
...
def main():
    ...
    siemplify.script_name = CREATE_RPZ_A_RULE_SCRIPT_NAME
    ...
    ipv4addr = extract_action_param(
        siemplify, param_name="IPv4 Address", input_type=str, is_mandatory=True, print_value=True
    )
    ...
    output_message = ""
    object_identifier = CREATE_RPZ_A_RULE_ACTION_IDENTIFIER_DOMAIN
    record_type = "record:rpz:a"
    ...
    try:
        ...
        validate_ip_address(validate_required_string(ipv4addr, "IPv4 Address"), "IPv4 Address", 4)
        ...
        if object_type == "IP Address":
            object_identifier = CREATE_RPZ_A_RULE_ACTION_IDENTIFIER_IP
            record_type = "record:rpz:a:ipaddress"

        result = api_manager.create_rpz_a_rule(
            object_identifier, name, rp_zone, ipv4addr, comment, additional_parameters
        )
        siemplify.result.add_result_json(json.dumps(result, indent=4))

        if result:
            table_data = [RPZRuleRecord(result, record_type).create_a_rule_csv()]
            siemplify.result.add_data_table(
                title="RPZ A Rule", data_table=construct_csv(table_data)
            )

            output_message = f"Created RPZ A rule {name}."

        else:
            result_value = RESULT_VALUE_FALSE
            output_message = "Failed to create RPZ A rule."
        ...
        status = EXECUTION_STATE_FAILED
        output_message = COMMON_ACTION_ERROR_MESSAGE.format(CREATE_RPZ_A_RULE_SCRIPT_NAME, str(e))
        result_value = RESULT_VALUE_FALSE
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
